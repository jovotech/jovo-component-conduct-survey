...
const surveyHandler: Handler = {
    START() {
        ...
        this.$speech.t('component-ConductSurvey.start', {numberOfQuestions: this.getActiveComponent()!.config.numberOfQuestions});
        this.$speech.t(`component-ConductSurvey.questions.${this.$session.$data[this.getActiveComponent()!.name].questionCount}`);
    ...
    AnswerIntent() {
        ...
        if (questionCount >= this.getActiveComponent()!.config.numberOfQuestions ||
            questionCount >= this.t('component-ConductSurvey.questions').length) {

            this.getActiveComponent()!.data.answers = this.$session.$data[this.getActiveComponent()!.name].answers;
